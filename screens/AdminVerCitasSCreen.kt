...
fun AdminVerCitasScreen(navController: NavHostController, modoPeluquero: Boolean = false) {
    ...
    var estadoSeleccionado by remember { mutableStateOf(citaSeleccionada?.estado ?: "pendiente") }

    var clientes by remember { mutableStateOf<List<Cliente>>(emptyList()) }
    var servicios by remember { mutableStateOf<List<Map<String, Any>>>(emptyList()) }
    val showConfirmCancelarDesdeFormulario = remember { mutableStateOf(false) }
    ...
    if (showConfirmCancelarDesdeFormulario.value && citaSeleccionada != null) {
        AlertDialog(
            onDismissRequest = { showConfirmCancelarDesdeFormulario.value = false },
            title = { Text("¿Cancelar cita?") },
            text = { Text("Esta acción eliminará la cita. ¿Deseas continuar?") },
            confirmButton = {
                TextButton(onClick = {
                    FirebaseService.eliminarReserva(
                        negocioId = negocioId!!,
                        ...
                        onSuccess = {
                            recargarCitas()
                            showConfirmCancelarDesdeFormulario.value = false
                        ...
                        onFailure = { Log.e("Reserva", "Error al eliminar: ${it.message}") }
                    )
                }) {
                    Text("Sí, cancelar")
                }
            ...
            dismissButton = {
                TextButton(onClick = { showConfirmCancelarDesdeFormulario.value = false }) {
                    Text("No")
                }
            }
        )
        ...
        AlertDialog(
            ...
            text = {
                Column(modifier = Modifier.fillMaxWidth()) {
                    ...
                    DropdownMenuBox(
                        opciones = listOf("pendiente", "cancelada", "finalizada"),
            ...
            confirmButton = {
                TextButton(onClick = {
                    ...
                    if (estadoSeleccionado == "cancelada" && citaSeleccionada != null) {
                        showConfirmCancelarDesdeFormulario.value = true
                        return@TextButton
                    }
